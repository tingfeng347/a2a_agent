...
from config import (
    LOGISTICS_AGENT_PORT,
    ORDER_AGENT_PORT,
    POLICY_AGENT_PORT,
    TRIAGE_AGENT_PORT,
)
from customer_console import main as console_main
from shared.debug import debug_log, is_debug_enabled


ROOT_DIR = Path(__file__).resolve().parent

AGENT_SPECS = [
    ("Triage Agent", "triage_agent.py", TRIAGE_AGENT_PORT),
    ("Order Agent", "order_agent.py", ORDER_AGENT_PORT),
    ("Policy Agent", "policy_agent.py", POLICY_AGENT_PORT),
    ("Logistics Agent", "logistics_agent.py", LOGISTICS_AGENT_PORT),
]
...
def main() -> None:
    ...
    try:
        if is_debug_enabled():
            debug_log("Launcher", f"工作目录={ROOT_DIR}")

        started_processes = start_agents()
        wait_for_agents()
        print("[launcher] 客服多智能体已就绪，进入控制台。输入 exit 退出。")
